...
export async function getOrder({ token, orderId }: { token: string; orderId: string }) {
   const res = await axiosClient.get<OrderResponseType>(`/order/${orderId}`, {
...
export default function useGetOrder({ enabled, token, orderId }: { enabled: boolean; token: string; orderId: string }) {
   return useQuery({
